...
- **Section Apprendre** : 10 articles statiques (`src/lib/articles.ts`), 3 catégories (bases/nutrition/recettes ; Mindset supprimée). Accueil = carte à la une + **une rangée horizontale (carrousel) par thème** pour éviter l'effet « feed sans fin » sur mobile ; « Voir tout › » ouvre la liste complète du thème ; recherche/filtres basculent sur la liste verticale. Cartes illustrées par une **photo Unsplash thématique** par article (`ARTICLE_IMAGE` + `articleThumb()` dans articles.ts), plus l'emoji conservé pour le header du détail. Recettes : petits-déj, déjeuners express, dîners légers, batch cooking
...
- **Suivi du poids** : table `weight_logs` (1/jour, upsert). Carte dashboard = aperçu (poids actuel + **variation totale depuis le début** + sparkline) cliquable vers `/weight`, avec un « + » de saisie rapide (bottom sheet). **Page `/weight`** : grande courbe (aire+ligne), stats (départ, variation totale, rythme/semaine colorés selon objectif), historique **modifiable** (tap = éditer une pesée passée, corbeille = supprimer avec confirmation inline). Actions dans `weight/actions.ts` (`saveWeight`/`deleteWeight`) : resynchronisent `profiles.weight_kg` sur la pesée la plus récente → recalcul auto de l'objectif calorique. Pas de poids objectif pour l'instant (proposé mais non retenu → barre de progression/ETA en attente)
